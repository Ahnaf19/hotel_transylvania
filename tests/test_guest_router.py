...
class TestGuestRouter:
    ...
    def setup_class(cls) -> None:
        ...
        cls.update_data = {
            ...
            "guest_contact": "01700000002"
        }
        cls.guest_id = "23159162-dd67-4a2a-8054-d6be6c0379ca"
        # Convert dummy_guest_data to a dictionary to match with FastAPI JSON response
                    ...
                    "guest_history": [
                        {
                            ...
                            "guest_departure": str(history.guest_departure) if history.guest_departure else None
    ...
    def test_read_guest(self) -> None:
        ...
        assert response.status_code == 200
        assert response.json() == self.dummy_guest_data_converted["guests"][self.guest_id]
    ...
    def test_read_guest_not_found(self) -> None:
        ...
        assert response.json()["detail"]["error_message"] == f"Guest with id {guest_id} not found"
    ...
    def test_add_guest_already_exists(self) -> None:
        ...
        assert response.json()["detail"]["error_message"] == f"Guest with id {existing_guest['guest_id']} already exists"

    def test_modify_guest(self) -> None:
        """
        Test modifying an existing guest.
        """
        updated_guest = UpdateGuest(guest_name=self.update_data["guest_name"], guest_contact=self.update_data["guest_contact"])
        response = self.client.put(f"/guests/{self.guest_id}", json=updated_guest.model_dump())
        assert response.status_code == 200
    ...
    def test_modify_guest_not_found(self) -> None:
        ...
        guest_id = "nonexistent-id"
        updated_guest = UpdateGuest(guest_name=self.update_data["guest_name"], guest_contact=self.update_data["guest_contact"])
        response = self.client.put(f"/guests/{guest_id}", json=updated_guest.model_dump())
        assert response.status_code == 404
        assert response.json()["detail"]["error_message"] == f"Guest with id {guest_id} not found"
